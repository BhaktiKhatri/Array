...
//        });
        ...
        for(int i=0; i<intervals.size()-1; i++) {
        	System.out.println("i: "+i+" intervals.get(i).end: "+intervals.get(i).end+" intervals.get(i+1).start: "+intervals.get(i+1).start);
            
        	if(intervals.get(i).end >= intervals.get(i+1).start) {		//if overlap then updated current interval end with next interval end
                intervals.get(i).end = Math.max(intervals.get(i).end,intervals.get(i+1).end);
                intervals.remove(i+1);	//remove the next interval
                i--;
            }
        }
